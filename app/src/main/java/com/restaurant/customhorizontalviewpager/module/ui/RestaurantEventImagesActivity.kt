...
import kotlinx.android.synthetic.main.activity_restaurant_event_images.*
import org.parceler.Parcels


class RestaurantEventImagesActivity : DialogBaseActivity() {


    private var eventList: ArrayList<String>? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_restaurant_event_images)
    ...
    private fun openBundle() {
        intent.extras?.let {
            eventList = Parcels.unwrap(it.getParcelable<Parcelable>(Constants.BundleKeys.RESTAURANT_EVENT_IMAGES))
        }
    }

    private fun initUI() {
        eventList?.let { eventList ->
            event_images_big_layout_rv.layoutManager = LinearLayoutManager(
                this, LinearLayoutManager.HORIZONTAL, false
            )
            event_images_big_layout_rv.adapter = RestaurantEventsAdapter(eventList,Constants.IMAGE_LAYOUT.BIG) { item ->
                //do nothing
            }

            val snapHelper: SnapHelper = RecyclerViewSnapHelper()
            snapHelper.attachToRecyclerView(event_images_big_layout_rv)
        }
    }
